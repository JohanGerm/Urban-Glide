...
class LoginMock extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: Text('Mock login')), body: Padding(padding: EdgeInsets.all(16), child: Column(children: [
      ...
      ElevatedButton(onPressed: () async { final uc = await FirebaseAuth.instance.signInAnonymously(); await FirebaseDatabase.instance.ref('users/${uc.user!.uid}').set({'phone': _phone.text.trim(), 'role':'rider'}); }, child: Text('Sign in (mock)'))
    ])));
  }
}
...
class _PanicHomeState extends State<PanicHome> {
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: Text('Rider Panic')), body: Padding(padding: EdgeInsets.all(16), child: Column(children: [
